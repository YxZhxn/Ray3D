...
class Trainer():
        ...
        with torch.no_grad():
            ...
            for cam, batch, batch_2d in test_generator.next_epoch():
                ...
                if return_predictions:
                    # -------- eval_data_prepare --------
                    inputs_2d_p = torch.squeeze(inputs_2d)
                    out_num = inputs_2d_p.shape[0] - self.receptive_field + 1
                    eval_input_2d = torch.empty(out_num, self.receptive_field, inputs_2d_p.shape[1], inputs_2d_p.shape[2])
                    for i in range(out_num):
                        eval_input_2d[i, :, :, :] = inputs_2d_p[i:i + self.receptive_field, :, :]
                    inputs_2d = eval_input_2d
                    # -----------------------------------
                    if self.model_config['TRAJECTORY_MODEL']:
                        return (
                ...
                if not self.model_config['TRAJECTORY_MODEL']:
                    batch[:, :, 1:] -= batch[:, :, 0:1]
                    batch[:, :, 0] = 0
                inputs_3d = torch.from_numpy(batch.astype('float32'))
                inputs_2d, inputs_3d = self.eval_data_prepare(self.receptive_field, inputs_2d, inputs_3d)
    ...
    def evaluate(self, mlog, subjects_test, pose_data, action_filter, pad, causal_shift, epoch, plot=False):
        ...
        if 'CAMERA_WISE_PERFORMANCE' in self.data_config and self.data_config['CAMERA_WISE_PERFORMANCE']:
            ...
            for cam_idx in range(len(camera_dist)):
                ...
                # mlog.info('CAM ID {}, pitch {}, translation_scale {}, degree {}, {} {} {} {}'.format(cam_id, pitch, translation_scale, degree, p1, p2, p3, p4))
        else:
            for action_key in all_actions.keys():
    ...
    def render(self, dataset, keypoints, keypoints_metadata,
        ...
        prediction = self.evaluate_core(render_generator, return_predictions=True).squeeze(axis=1)

        if ground_truth is not None and not self.model_config['TRAJECTORY_MODEL']:
            # Reapply trajectory
        ...
        if ground_truth is not None:
            prediction = cam.camera2world(prediction)
            ground_truth = cam.camera2world(ground_truth)
        else:
