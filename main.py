...
def main():
    ...
    mlog.info('PATH GT 2D: {}'.format(data_config['GT_2D']))
    ...
    poses_train, poses_train_2d, _, cameras_train = pose_data.fetch_via_subject(subjects_train, action_filter,
                                                                             subset=data_config['SUBSET'])
    poses_valid, poses_valid_2d, _, cameras_valid = pose_data.fetch_via_subject(subjects_test, action_filter)
    ...
    train_delegator = Model(model_config, is_train=True)
    test_delegator = Model(model_config, is_train=False)
    models = {
    ...
    if not models['train_trj'] is None:
        trainable_params.append(
            {'name': 'trj_model', 'params': models['train_trj'].parameters(), 'lr': train_config['LEARNING_RATE']}
        )
