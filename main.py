...
def main():
    ...
    train_delegator = Model(model_config, data_config)
    test_delegator = Model(model_config, data_config)
    models = {
    ...
    if train_config['OPTIMIZER'] == 'rmsprop':
        optimizer = optim.RMSprop(trainable_params, lr=train_config['LEARNING_RATE'])
